...
20) La conclusión a la que se llega, con este modelo nuevo, es que a veces es preferible que un clasificador detecte los grupos clave en vez de detectar los grupos irrelevantes, aunque esto tenga costos con las demás clases. Se puede concluir que hay varias formas de representar estos pesos y de generar modelos más acordes con las necesidades del problema y que este desbalanceo puede usarse para obtener un mejor resultado.
...
23) Al usar K-means, para el clustering en el problema de *Churn*, podría ayudar porque al generarse un nuevo atributo calculado algunas propiedades del usuario, provenientes del dataset, este valor generado sirva para que modelos lo tomen en cuenta en sus atributos relevantes, como por ejemplo, puede servir de condición para ver que ramificación toma un arbol de decisión.